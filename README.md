...
- A `sources` array — each config source (network response, DOM script, JS chunk) with the clean URLs extracted from it and any parse `error`. When at least one source failed to parse, a `source_errors` map alongside it gives the decode failure per origin, stored once there rather than repeated on every URL that source contributed.
